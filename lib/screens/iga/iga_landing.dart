...
class _IgaLandingState extends State<IgaLanding> {
  ...
  Widget buildIgaList(BuildContext context) {
    return ListView(
      children: igaList.asMap().entries.map((entry) {
        ...
        return buildIgaListItem(context, item, isFirst, isLast, height);
      }).toList(),
    );
  }

  // Method to build individual list item
  Widget buildIgaListItem(BuildContext context, Map<String, dynamic> item,
      bool isFirst, bool isLast, double height) {
    return Column(
      children: <Widget>[
  ...
  Future<void> checkInternet(BuildContext context, ConnectionStatus conn) async {
    ...
    if (conn.isOnline && connectionStatus.contains('disconnected')) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        SnackbarUtil.showSnackBar(context, 'Internet yagarutse!',
            const Color.fromARGB(255, 0, 255, 85));
      });
      connectionStatus.remove('disconnected');
    }
  }
}
