...
class _IgaContentState extends State<IgaContent> {
  ...
  void _processFinishedProgresses(List<CourseProgressModel?> progresses) async {
    ...
    if (nextIsomo?.id != null) {
      IngingoService().getTotalIsomoIngingos(widget.isomo.id)
          .listen((event) {
      setState(() {
        nextIsomoTotalIngingos = event.realTotalIngingos;
      });
    });
    }
  }
            ...
            builder: (context, popQuestions, _) {
              return Consumer<List<IngingoModel>?>(
                builder: (context, ingingos, _) {
                  ...
                  return loadingNextIsomo ? const LoadingWidget() : _buildContent(
                      context, currentIngingo, totalIngingos, unansweredPopQuestions, popQuestions, usr);
                },
              );
            },
          );
  ...
  Widget _buildIngingosScreen(BuildContext context, int currentIngingo, int totalIngingos) {
  ...
  void _startNextIsomo(BuildContext context, List<PopQuestionModel>? popQuestions, User? usr) {
    Navigator.pop(context);
    showDialog(
      context: context,
