...
class _ProcessingIshyuraState extends State<ProcessingIshyura> {
  ...
  Future<void> _loadPaymentData() async {
    try {
      ...
      setState(() => error = 'Failed to load payment data: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          _buildYellowContainer(context, message),
          _buildForm(context),
  ...
  Widget _buildYellowContainer(BuildContext context, String message) {
    return Container(
  ...
  Widget _buildConfirmButton(BuildContext context) {
    return Container(
      ...
      child: Align(
        ...
        child: Text(
          widget.ifatabuguzi.type == 'ur' ? 'CONFIRM' : 'EMEZA KWISHYURA',
          textAlign: TextAlign.center,
          style: TextStyle(
  ...
  Future<void> _handlePayment(BuildContext context) async {
    ...
    PaymentModel? payment = usr != null
        ? PaymentModel(
            ...
            igiciro: widget.ifatabuguzi.igiciro,
            isApproved: false,
            phone: phone)
        : null;
    ...
    if (_formKey.currentState!.validate() && payment != null) {
      ...
      // PAY THE MONEY
      try {
        ReturnedResult payRes = await PaymentService().createPayment(payment);
        ...
        if (payRes.value == true) {
          ...
          showDialog(
              ...
              builder: (BuildContext context) {
                return ItsindireAlert(
                    errorTitle: widget.ifatabuguzi.type == 'ur'
                        ? 'Pay via MOMO'
                        : 'Ishyura na MOMO',
                    errorMsg: widget.ifatabuguzi.type == 'ur'
          ...
          setState(() => error = payRes.error ?? 'An error occurred');
        }
      } catch (e) {
    ...
    if (error.isNotEmpty) {
      showDialog(
          ...
          builder: (BuildContext context) {
            return ItsindireAlert(
                errorTitle: 'Error', errorMsg: error, alertType: 'error');
          });
    }
  }
}
